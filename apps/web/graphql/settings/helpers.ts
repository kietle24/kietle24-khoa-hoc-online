...
export const checkForInvalidPaymentMethodSettings = (
    ...
    if (
        siteInfo.paymentMethod === UIConstants.PAYMENT_METHOD_PAYOS &&
        !(
            (siteInfo.payosClientId || process.env.PAYOS_CLIENT_ID) &&
            (siteInfo.payosApiKey || process.env.PAYOS_API_KEY) &&
            (siteInfo.payosChecksumKey || process.env.PAYOS_CHECKSUM_KEY)
        )
    ) {
        failedPaymentMethod = UIConstants.PAYMENT_METHOD_PAYOS;
    }
